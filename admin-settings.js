...
const monsterLunchLevelInputs = [1, 2, 3, 4].map((level) => ({
  goal: document.getElementById(`adminMonsterGoalL${level}`),
  missesAllowed: document.getElementById(`adminMonsterMissesL${level}`),
  choiceCount: document.getElementById(`adminMonsterChoicesL${level}`),
  foodSize: document.getElementById(`adminMonsterSizeL${level}`),
  timeLimitEnabled: document.getElementById(`adminMonsterTimerEnabledL${level}`),
  timeLimit: document.getElementById(`adminMonsterTimeL${level}`),
  moveSpeed:
    level >= 3
      ? document.getElementById(`adminMonsterSpeedL${level}`)
      : null,
  snatcherDelaySeconds:
    level >= 3
      ? document.getElementById(`adminMonsterSnatcherL${level}`)
      : null,
}));
...
const resetMonsterLunchDefaultsBtn = document.getElementById("resetMonsterLunchDefaultsBtn");
...
const DEFAULT_MONSTER_LUNCH_LEVELS = [
  {
    goal: 5,
    missesAllowed: 3,
    choiceCount: 3,
  ...
  return levelInputs.map((inputs) => {
    ...
    Object.entries(fieldMap).forEach(([levelKey, inputKey]) => {
      ...
      if (input.type === "checkbox") {
        level[levelKey] = input.checked;
        return;
      }

      level[levelKey] = input.value;
    });
...
function applyGameLevelsToInputs(levels, levelInputs, fieldMap) {
  levelInputs.forEach((inputs, index) => {
    ...
    Object.entries(fieldMap).forEach(([levelKey, inputKey]) => {
      ...
      if (input.type === "checkbox") {
        input.checked = level[levelKey] === true;
        return;
      }

      input.value = String(level[levelKey]);
    });
  });
}
...
function normalizeMonsterLunchLevels(levels) {
  return normalizeLevelArray(
    levels,
    DEFAULT_MONSTER_LUNCH_LEVELS,
    (level, defaults) => ({
      goal: parseLightTapLevelValue(
        level.goal,
        1,
      ...
      choiceCount: parseLightTapLevelValue(
        level.choiceCount,
        3,
        10,
      ...
      foodSize: parseLightTapLevelValue(
        level.foodSize,
        60,
        140,
        defaults.foodSize
...
function loadStoredMonsterLunchLevels() {
  return loadGameLevelsFromStorage(
    "monsterLunchLevels",
    DEFAULT_MONSTER_LUNCH_LEVELS,
    normalizeMonsterLunchLevels
  );
}

function saveStoredMonsterLunchLevels(levels) {
  return saveGameLevelsToStorage(
    "monsterLunchLevels",
    levels,
    normalizeMonsterLunchLevels
  );
...
async function resetFullscreenToDefaults() {
  ...
  localStorage.setItem(
    HAUNTED_STREET_THROW_INTERVALS_KEY,
    JSON.stringify(hauntedStreetLevelThrowIntervals)
  );
  localStorage.setItem(
...
async function loadTask1Settings() {
  ...
  let monsterLunchLevels = loadStoredMonsterLunchLevels();
  ...
  let hauntedStreetLevelThrowIntervals =
    DEFAULT_HAUNTED_STREET_THROW_INTERVALS.map((fallback, index) => {
      try {
        const stored = JSON.parse(
          localStorage.getItem(HAUNTED_STREET_THROW_INTERVALS_KEY) ||
            "[]"
        );
        const value = Number.parseInt(stored[index], 10);
        return Number.isFinite(value)
          ? Math.min(3000, Math.max(150, value))
          : fallback;
      } catch {
        return fallback;
      }
  ...
  let hauntedStreetLevelPumpkinSpeeds =
    DEFAULT_HAUNTED_STREET_PUMPKIN_SPEEDS.map((fallback, index) => {
      try {
        const stored = JSON.parse(
          localStorage.getItem(HAUNTED_STREET_PUMPKIN_SPEEDS_KEY) ||
            "[]"
        );
        const value = Number.parseInt(stored[index], 10);
        return Number.isFinite(value)
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (Array.isArray(data.hauntedStreetLevelThrowIntervals)) {
        hauntedStreetLevelThrowIntervals =
          DEFAULT_HAUNTED_STREET_THROW_INTERVALS.map((fallback, index) => {
            const value = Number.parseInt(
              data.hauntedStreetLevelThrowIntervals[index],
              10
            );
            return Number.isFinite(value)
              ? Math.min(3000, Math.max(150, value))
      ...
      if (Array.isArray(data.hauntedStreetLevelPumpkinSpeeds)) {
        hauntedStreetLevelPumpkinSpeeds =
          DEFAULT_HAUNTED_STREET_PUMPKIN_SPEEDS.map((fallback, index) => {
            const value = Number.parseInt(
              data.hauntedStreetLevelPumpkinSpeeds[index],
              10
            );
            return Number.isFinite(value)
              ? Math.min(1200, Math.max(150, value))
      ...
      if (
        Array.isArray(data.monsterLunchLevels)
      ) {
        monsterLunchLevels =
          normalizeMonsterLunchLevels(
            data.monsterLunchLevels
          );

        saveStoredMonsterLunchLevels(
          monsterLunchLevels
        );
      }
  ...
  hauntedStreetLevelInputs.forEach((inputs, index) => {
    ...
    if (inputs.throwInterval) {
      inputs.throwInterval.value = String(
        hauntedStreetLevelThrowIntervals[index]
      );
    }
...
async function saveTask1Settings() {
  ...
  const hauntedStreetLevelThrowIntervals =
    hauntedStreetLevelInputs.map((inputs, index) => {
      const value = Number.parseInt(
        inputs.throwInterval ? inputs.throwInterval.value :
          DEFAULT_HAUNTED_STREET_THROW_INTERVALS[index],
        10
      );
      return Number.isFinite(value)
        ? Math.min(3000, Math.max(150, value))
  ...
  const hauntedStreetLevelPumpkinSpeeds =
    hauntedStreetLevelInputs.map((inputs, index) => {
      const value = Number.parseInt(
        inputs.pumpkinSpeed ? inputs.pumpkinSpeed.value :
  ...
  saveStoredMonsterLunchLevels(monsterLunchLevels);
